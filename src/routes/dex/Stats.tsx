	...
	return (
		...
		>
			{item.teammates.map((x, i) => (
				<MotionListItem key={i} initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
					<ListItemAvatar>
						<Avatar
							alt={'?'}
							imgProps={{ crossOrigin: 'anonymous' }}
						...
						{x.types.map((x, i) => (
							<Avatar
								...
								imgProps={{ crossOrigin: 'anonymous' }}
								src={apiUrl(`/sprites/static/typesNew/${x.nameDetails.translations.en}.svg`)}
								sx={{
									position: 'absolute',
									top: 15,
									left: 200 + i * 20,
				...
				<MotionListItem key={i} initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
					<ListItemAvatar>
						<Avatar
							...
							imgProps={{ crossOrigin: 'anonymous' }}
							src={apiUrl(
								`/sprites/static/teratypes/${x.type[0].toUpperCase() + x.type.slice(1)}.png`
							)}
						/>
					</ListItemAvatar>
					<ListItemText primary={x.name} secondary={`${x.usage}%`} />
...
const DetailSection: React.FC<PropsWithChildren<{ title: string }>> = ({ title, children }) => {
	return (
