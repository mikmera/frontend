...
export const CreateSets: React.FC = () => {
	...
	const [tableRows, setTableRows] = React.useState<{ label: string; value: number }[]>([])
	...
	React.useEffect(() => {
		const fetchData = async () => {
			const { pokemon: data } = await fetcher(apiUrl(`/v1/autocomplete/pokemon/${pokemon}`))
			if (!data) return
	...
	React.useEffect(() => {
		...
		const correction =
			natures.find((v) => v.label === nature)?.correction || natureInit[0].correction
		if (!stats) return
		...
		const calculateStat = (
			...
			if (index === 0) return Math.floor((base * 2 + iv + effort / 4) * 0.5 + 60)
			else return Math.floor(((base * 2 + iv + effort / 4) * 0.5 + 5) * statCorr)
		}
	...
	React.useEffect(() => {
		const fetchData = async () => {
			const { abilities: data } = await fetcher(apiUrl(`/v1/autocomplete/abilities/${pokemon}`))
			if (!data) return
		...
		<Box sx={{ width: '100%', display: 'flex', justifyContent: 'center' }}>
			{!data.pokemon || !data.items ? (
				...
				<Grid container sx={{ width: '450px', m: 3 }}>
					...
					<MoveSelector moves={moves} moveList={moveList} disabled={disabled} setMove={setMove} />
					<TeraTypeAutoComplete teraTypes={teraTypes} setTerastal={setTeraType} />
					...
					{statKeys.map((stat, index) => (
						<StatSlider
							...
							correction={natures.find((v) => v.label === nature)?.correction[statEn[index]]}
						/>
					))}
					<Grid item xs={12}>
						<StatTable stats={tableRows} />
					</Grid>
					<LoadingButton sx={{ mt: 2 }} variant="contained" onClick={Upload} loading={waitUpload}>
						샘플 올리기
					</LoadingButton>
				</Grid>
			)}
