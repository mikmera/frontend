...
export const Nav: React.FC = wrapError(() => {
	...
	const closeDrawer = React.useCallback(() => setDrawerOpen(false), [setDrawerOpen])

	const toggleDrawer = React.useCallback(() => setDrawerOpen((v) => !v), [setDrawerOpen])
		...
		<>
			...
			<AppBar color="inherit" sx={{ zIndex: (theme) => theme.zIndex.drawer + 1 }}>
				<Toolbar>
					...
					<Box sx={{ mr: 5 }}>
						<FormControlLabel
							control={<ToggleDarkmode sx={{ m: 1 }} defaultChecked={defaultChecked} />}
							label={isMobile ? '' : theme === 'dark' ? '루나톤모드' : '솔록모드'}
							onChange={() => {
								toggleThemeMode()
					...
					{isMobile ? (
						...
						<LayoutGroup>
							<Stack direction="row" alignItems="center" spacing={6} height="100%">
								{navLinks.map((x, i) => (
