...
const DexItem: React.FC<{ item: UsageData; index: number }> = wrapError(({ item, index }) => {
	const {
		data: { type }
	} = useDexContext()
	...
	const iconUrl = React.useMemo(
		...
		[item.pokemon.dexId, item.pokemon.formId]
	)
	...
	return (
		<ListItemButton component={Link} to={`/dex/${type}/${item.pokemon.id}`}>
			...
			<ListItemAvatar>
				<Avatar alt="image" imgProps={{ crossOrigin: 'anonymous' }} src={iconUrl} variant="rounded">
					<QuestionMark />
				</Avatar>
			</ListItemAvatar>
			<ListItemText
...
export const MainSidebar: React.FC = () => {
	...
	const loadMore = React.useCallback(async () => {
		...
		setLoading(false)
	}, [data.count, data.type, data.usages?.length, update])
