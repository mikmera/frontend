...
const ErrorCode = {
	'401': {
		title: '로그인이 필요해요',
		message: '이곳은 트레이너 전용 구역이에요! 어서 포켓몬센터의 PC에서 로그인해주세요'
	},
	'403': {
		title: '접근 권한이 없어요',
		message: '여기는 탐험할 수 없는 구역이에요! 접근하려면 특별한 배지가 필요할 것 같아요'
	},
	'404': {
		title: '페이지를 찾을 수 없어요',
	...
	'500': {
		title: '서버에 문제가 발생했어요',
		message: '서버가 혼란에 빠져 영문도 모른체 자신을 공격한것 같아요, 잠시후 다시 시도해보세요'
	},
	'503': {
	...
	return (
		<div className="figure">
			<div className="error-no">
				<span className={`error-digit ${isMobile ? 'mobile' : ''}`}>{code?.[0]}</span>
				{/** when code[1] === 0 then pokeball else just code[1]*/}
				{code?.[1] === '0' ? (
					<Pokeball />
				) : (
					<span className={`error-digit ${isMobile ? 'mobile' : ''}`}>{code?.[1]}</span>
				)}
				<span className={`error-digit ${isMobile ? 'mobile' : ''}`}>{code?.[2]}</span>
			</div>
